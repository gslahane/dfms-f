...
const FundDemandDetails = ({
  ...
  // Calculations
  const grossTotal =
    (work.adminApprovedAmount ?? 0) + (work.taxDeductionAmount ?? 0);
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div>
                ...
                <div>
                  {vendor?.aadhar
                    ? `XXXX-XXXX-${vendor.aadhar.slice(-4)}`
                    : "N/A"}
                </div>
              </div>
            </div>
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-3 text-sm mb-3">
              ...
              <div>
                ...
                <div>₹{(demand.amount ?? 0).toLocaleString()}</div>
              </div>
              <div>
                <span className="font-medium text-gray-600">Net Payable</span>
                <div>
                  ₹{(demand.netPayable ?? demand.amount ?? 0).toLocaleString()}
                </div>
              </div>
              <div>
                ...
                <tbody>
                  {workDemands.map((d: any) => (
                    ...
                    >
                      ...
                      <td className="px-2 py-1 border">
                        ₹{(d.netPayable ?? d.amount ?? 0).toLocaleString()}
                      </td>
                      <td className="px-2 py-1 border">
