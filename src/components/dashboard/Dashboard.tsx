...
const mlaMlcData = {
  ...
  data: [
    ...
    { district: "PUNE", taluka: "Khed Alandi", type: "MLA", term: 15, name: "Babaji Ramchandra Kale", schemeName: "45150012 53 MLA/MLC LOCAL DEVELOPMENT PROGRAMME", budget: 50000, fundUtilized: 0, balance: 50000, pendingDemands: 0 },
...
const Dashboard: React.FC = () => {
  ...
  const districtOptions = useMemo(
    () => ["All", ...uniq(dapPayload.data.map(d => d.district))],
    []
  );
  ...
  const mlaDistrictOptions = useMemo(
    () => ["All", ...uniq(mlaMlcData.data.map(d => d.district))],
    []
  );
  ...
  // — Filtered DAP rows (with pending at end, balance before pending)
  const rowsDAP = useMemo(() => {
    let filtered = dapPayload.data
      .filter(d =>
    ...
    // Sort: pending demands (demandsPendingForApproval > 0) at end
    filtered = [
      ...filtered.filter(r => !r.demandsPendingForApproval || r.demandsPendingForApproval === 0),
      ...filtered.filter(r => r.demandsPendingForApproval > 0)
    ];
    return filtered;
  }, [fy, district, demand, showPendingOnly]);

  // — Filtered MLA/MLC rows (with pending at end, balance before pending)
  const rowsMLRaw = useMemo(() => {
    let filtered = mlaMlcData.data
      .filter(d => mlaDistrict === "All" || d.district === mlaDistrict)
      .filter(d => mlaName === "All" || d.name === mlaName)
      .filter(d => !showPendingOnly || d.pendingDemands > 0);

    filtered = [
      ...filtered.filter(r => !r.pendingDemands || r.pendingDemands === 0),
      ...filtered.filter(r => r.pendingDemands > 0)
    ];
    return filtered;
  }, [mlaDistrict, mlaName, showPendingOnly]);

  // — Aggregated HADP rows (filtered & pending last, balance before pending)
  const rowsH = useMemo(() => {
    ...
    arr = [
      ...arr.filter(r => !r.balance || r.balance === 0),
      ...arr.filter(r => r.balance > 0)
    ];
    return arr;
    ...
    <div className="p-4 space-y-6">
      ...
      <div className="flex flex-wrap gap-3 text-sm">
        ...
        {planType === "ML" && (
          <>
            <div className="flex-1 min-w-[180px]">
              <Label htmlFor="mlaDistrict">District</Label>
              <Select
                id="mlaDistrict"
                value={mlaDistrict}
        ...
        {planType === "HADP" && (
          <>
            <div className="flex-1 min-w-[180px]">
              <Label htmlFor="hadpDistrict">District</Label>
              <Select id="hadpDistrict" value={hadpDistrict} onValueChange={setHadpDistrict}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {hadpDistrictOptions.map(o => <SelectItem key={o} value={o}>{o}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div className="flex-1 min-w-[180px]">
              <Label htmlFor="hadpTaluka">Taluka</Label>
              <Select id="hadpTaluka" value={hadpTaluka} onValueChange={setHadpTaluka}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {hadpTalukaOptions.map(o => <SelectItem key={o} value={o}>{o}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          </>
        ...
        <Card className="rounded-md shadow-sm">
          <CardHeader>
            <CardTitle className="text-sm">
              MLA/MLC Details <span className="text-red-500"></span>
                        <div className="flex gap-2 text-right">
            <span className="text-[10px] text-red-500">*Balance = Budget Amount- Fund Distributed |</span>
            <span className="text-[10px] text-red-500">*Amounts In Thousand</span>
          </div>
            ...
            <Table className="min-w-full border border-gray-300">
              ...
              <TableBody>
                {rowsMLRaw.map((r, idx) => (
                  <TableRow key={`${r.name}|${idx}`}>
                    <TableCell className="border px-2 py-1 text-xs">{idx + 1}</TableCell>
                    <TableCell className="border px-2 py-1 text-xs">{r.district}</TableCell>
                    <TableCell className="border px-2 py-1 text-xs">{r.type}</TableCell>
              ...
              <TableFooter>
                <TableRow>
                  ...
                  <TableCell className="border px-2 py-1 text-right font-semibold">
                    {footerMLRaw.pendingDemands.toLocaleString()}
                  </TableCell>
                </TableRow>
              </TableFooter>
            </Table>
      ...
      {planType === "HADP" && (
        <Card className="rounded-md shadow-sm">
         <CardHeader>
  <CardTitle className="text-sm">HADP Summary<span className="text-xs text-red-300 italic">    *Amounts In Thousands</span></CardTitle>
